...
# Supplement missing distances
dist[('CNXGG', 'CNTAO')] = 452
dist[('INNSA', 'PKKHI')] = 560   # not needed now (INNSA->INMUN->PKKHI)
dist[('PKKHI', 'AEKLF')] = 605
dist[('INMUN', 'PKKHI')] = 246   # Mundra -> Karachi
...
<p class="info">ETA 1st Port: 14th April 2026 &nbsp;|&nbsp; CN Port Stay: 24h &nbsp;|&nbsp; IN Ports: 36h &nbsp;|&nbsp; KHOR FAKKAN: 48h &nbsp;|&nbsp; Man-in: 3h/port</p>
<table><thead><tr>
...
with open('voyage_schedule_v3_wait.html', 'w', encoding='utf-8') as f:
    f.write(gen_html(sched_wait,   '(With Waiting Time)', show_waiting=True))

with open('voyage_schedule_v3_nowait.html', 'w', encoding='utf-8') as f:
    f.write(gen_html(sched_nowait, '(No Waiting Time)',   show_waiting=False))

print("Done!")
print(f"Ports: {len(rotation)}")
print(f"With waiting  - Last ETD: {sched_wait[-1]['etd'].strftime('%d %b %Y %H:%M')}")
...
for i in range(1, len(rotation)):
    ...
    print(f"  {rotation[i-1]} -> {rotation[i]}: {d}")
