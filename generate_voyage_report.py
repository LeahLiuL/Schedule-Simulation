...
- Calculate voyage duration: current voyage home terminal berth -> next voyage same terminal berth
...
CSV_PATH = r'C:\Users\leahliu\WorkBuddy\Schedule-Simulation\shipping_data\bi_vessel_departure.csv'
HTML_PATH = r'C:\Users\leahliu\WorkBuddy\Schedule-Simulation\voyage_report.html'
...
def identify_home_terminal(vessel_df):
    ...
    for bound_pref in ['W', 'S', 'N', 'E']:
        ...
        if len(bound_recs) == 0:
            continue
        # For each voyage, find the first terminal by berth time
        first_terminals = []
...
def generate_report():
    ...
    # Identify home terminal for each vessel
    ...
    for vn in sorted(cul['vessel_name'].unique()):
        ...
        # Get sorted voyage numbers
        voyage_nums = sorted(vdf['voyage_num'].unique(), key=lambda x: int(x))

        # Get the start berth time for each voyage (at home terminal, primary bound)
        voyage_starts = {}
        for vnum in voyage_nums:
            ...
            if home_term:
                ...
                if len(home_recs) > 0:
                    ...
                    # Fallback: use the earliest berth of primary bound
                    if len(vnum_recs) > 0:
        ...
        for i, vnum in enumerate(voyage_nums):
            ...
            # Voyage start and end
            start_time = voyage_starts.get(vnum)
            # End time = next voyage's start time
